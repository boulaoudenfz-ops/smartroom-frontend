...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // The backend returns response with PHP comment, so we need to clean it
      let data = res.data
      ...
      // Handle the response structure: data.data.token or data.token
  ...
  />
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#4E342E' }}>Welcome back</h1>
          <p style={{ color: '#8D6E63', fontSize: '0.875rem', marginTop: '0.25rem' }}>Sign in to SmartRoom</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.875rem', color: '#8D6E63' }}>
          No account?{' '}
          <a href="/register" style={{ color: '#6D4C41', fontWeight: 500 }}>Create one</a>
        </p>
      </div>
    </div>
  )
